    ...
    document.addEventListener('DOMContentLoaded', function() {
            ...
            button.addEventListener('click', function() {
                ...
                if (this.classList.contains('view-business')) {
                    ...
                    if (firstCellText.startsWith('#USR-')) {
                        ...
                        userPhone = '(555) 123-4567'; // Placeholder as it's not in the dashboard table
        ...
        statusButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const row = this.closest('tr');
                if (card) {
                    itemName = card.querySelector('.approval-title, .review-user')?.textContent || 'this card';
                } else if (row) {
                    itemName = row.querySelector('td:nth-child(1), td:nth-child(2)')?.textContent || 'this user';
                }
        ...
        document.getElementById('applyUserFilters')?.addEventListener('click', function() {
            applyTableFilters('usersTable', 'userSearch', { status: document.getElementById('userStatusFilter').value });
        });
        ...
        document.getElementById('applyReviewFilters')?.addEventListener('click', function() {
            ...
            reviews.forEach(review => {
                let showReview = true;
                if (searchInput && !review.textContent.toLowerCase().includes(searchInput)) showReview = false;
                if (statusFilter !== 'all' && review.getAttribute('data-status') !== statusFilter) showReview = false;
                if (ratingFilter !== 'all' && review.getAttribute('data-rating') !== ratingFilter) showReview = false;
